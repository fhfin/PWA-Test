(async function() {
    ...
    function createCard(book) {
        ...
        img.src = book.image;
        // 如果没有图片，使用默认占位图或不显示，这里简单的设置一个宽高占位
        if (!book.image) {
        ...
        return data.items.map(function(item) {
            ...
            if (saleInfo) {
                if (saleInfo.saleability === 'FOR_SALE' && saleInfo.retailPrice) {
                    price = saleInfo.retailPrice.amount + ' ' + saleInfo.retailPrice.currencyCode;
                } else if (saleInfo.saleability === 'FOR_SALE' && saleInfo.listPrice) {
